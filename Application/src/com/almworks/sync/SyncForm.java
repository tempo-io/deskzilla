...
class SyncForm implements UIComponentWrapper {
  ...
  private final CollectionRenderer<SyncTask> myRenderer;
  ...
  public SyncForm(Configuration configuration, final ConfigAccessors.Bool showOnSync) {
    myConfiguration = configuration;
    myRenderer = new TaskRenderer();
    myProblemForm = new ProblemForm(myUIDetach);
...
//    myProblemsTable.setColumnModel(myProblemsTableColumns, true);
    ...
    myTasksList.setCellRenderer(myRenderer);
...
//    myToolbar.addAction(new IdActionProxy.ForToolbar(MainMenu.Connection.SYNCHRONIZE_FULL));
  ...
  static class ProblemForm implements ElementVisitor<JLabel> {
    ...
    private JPanel myWholePanel;
    private JTextPane myProblemDescription;
    ...
    public ProblemForm(DetachComposite uiDetach) {
    ...
    public void setProblem(SyncProblem problem) {
      if (problem != null) {
        ...
        ErrorHunt.setEditorPaneText(myProblemDescription, problem.getLongDescription());
        ...
        ErrorHunt.setEditorPaneText(myProblemDescription, "");
