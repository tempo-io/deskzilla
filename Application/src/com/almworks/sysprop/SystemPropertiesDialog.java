...
import com.almworks.util.components.etable.*;
...
public class SystemPropertiesDialog {
  ...
  static void showDialog(ActionContext context) throws CantPerformException {
    new SystemPropertiesDialog(context).showDialog();
  }
  ...
  private SystemPropertiesDialog(ActionContext context) {
    myContext = context;
  }

  private void showDialog() throws CantPerformException {
    myModel = createModel();
    myTable = createTable();

    final DialogBuilder builder =
      myContext.getSourceObject(DialogManager.ROLE).createMainBuilder("SystemPropertiesDialog");
  ...
  private JComponent createContent() {
    ...
    final int prefRows = Math.min(Math.max(myModel.getSize(), 10), 20);
