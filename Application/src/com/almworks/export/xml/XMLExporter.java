...
public class XMLExporter extends WriterExporterHelper {
  ...
  private final Map<ModelKey, String> myTagCache = Collections15.hashMap();
  ...
  private void writeIssues(PrintWriter out, ExportedData data, ExportParameters parameters) {
    ...
    for (ExportedData.ArtifactRecord record : records) {
      ...
      for (ModelKey<?> key : keys) {
        Pair<String, ExportValueType> pair = key.formatForExport(record.getValues(), numberFormat, dateFormat, false);
  ...
  private String getTag(ModelKey<?> key) {
    String tag = myTagCache.get(key);
    if (tag == null) {
      tag = key.getDisplayableName();
