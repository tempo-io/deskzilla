...
public class ExportAction extends SimpleAction implements Startable {
    ...
    WorkArea workArea, ProductInformation productInfo)
  {
    ...
    myDistributionType = productInfo.getDistributionType();
  }

  private static List<ExporterDescriptor> createExporterList(DialogManager dialogManager, WorkArea workArea,
    ProductInformation productInfo)
  {
    List<ExporterDescriptor> exporters = Collections15.arrayList();
    exporters.add(new CSVExporterDescriptor(dialogManager));
    exporters.add(new HTMLExporterDescriptor(dialogManager, workArea, productInfo));
    exporters.add(new XMLExporterDescriptor(dialogManager, productInfo));
    exporters.add(new PDFExporterDescriptor(dialogManager, productInfo, workArea));
  ...
  private ExportParameters collectParameters(ExportedData data, boolean silent) {
    ...
    ExportParameters parameters = dialog.collectParameters(silent);
    return parameters;
  }
