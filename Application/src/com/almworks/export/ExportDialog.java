...
class ExportDialog {
  ...
  private String getAttributesString() {
    Set<ModelKey<?>> keys = myTarget.getKeys();
    StringBuffer b = new StringBuffer();
    for (ModelKey<?> key : keys) {
      if (b.length() > 0)
        b.append(", ");
      b.append(key.getDisplayableName());
    }
    if (b.length() == 0)
  ...
  private void listenAttributesModel() {
    ChangeListener listener = new ChangeListener() {
      public void onChange() {
        List<ModelKey<?>> keys = myAttributesModel.toList();
        List<String> names = ModelKey.GET_NAME.collectList(keys);
  ...
  private SubsetModel<ModelKey<?>> createAttributesModel(
    Configuration config, ExportedData data)
  {
    final LinkedHashSet<ModelKey<?>> keys = data.getKeys();
    final Collection<Connection> conns = data.getConnections();

    final OrderListModel<ModelKey<?>> exportableKeys = OrderListModel.create();
    for(final ModelKey<?> key : keys) {
      if(key.isExportable(conns)) {
        exportableKeys.addElement(key);
      }
    }

    final SubsetModel<ModelKey<?>> subsetModel = SubsetModel.create(myLife, exportableKeys, false);
    ...
    if(names.size() > 0) {
      addKeys(subsetModel, keys, ModelKey.GET_NAME, names);
    } else {
      addKeys(subsetModel, keys, ModelKey.GET_DISPLAY_NAME, data.getSelectedColumnsNames());
    }

    return subsetModel;
  }

  private static void addKeys(SubsetModel<ModelKey<?>> subsetModel, LinkedHashSet<ModelKey<?>> allKeys,
    Convertor<ModelKey<?>, String> convertor, List<String> names)
  {
    Map<String, ModelKey<?>> map = convertor.assignKeys(allKeys);
    for (String name : names) {
      ModelKey<?> key = map.get(name);
      if (key != null) {
