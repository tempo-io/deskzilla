...
public class HTMLExporter extends WriterExporterHelper {
  ...
  public HTMLExporter(ExporterDescriptor descriptor, DialogManager dialogManager, Configuration configuration,
  ...
  private void writeDataTables(PrintWriter out, ExportedData data, ExportParameters parameters) {
    Set<ModelKey<?>> keys = parameters.getKeys();
    ...
    for (ExportedData.ArtifactRecord record : records) {
      ...
      String rowClasses = getRowClasses(record, keys, numberFormat, dateFormat);
      out.println("<tr>");
      for (ModelKey<?> key : keys) {
        Pair<String, ExportValueType> pair = key.formatForExport(record.getValues(), numberFormat, dateFormat, true);
  ...
  private void writeTableHeader(PrintWriter out, Set<ModelKey<?>> keys, int page, int totalPages, ExportedData data,
    ExportParameters parameters)
  {
    ...
    out.print("<tr>");
    for (ModelKey<?> key : keys) {
      out.print("<th>" + escape(key.getDisplayableName()) + "</th>");
    }
    out.println("</tr>");
  }
  ...
  private String getColClasses(ModelKey<?> key, ExportValueType type) {
    StringBuffer result = new StringBuffer();
    String name = Util.lower(key.getDisplayableName());
  ...
  private String getRowClasses(ExportedData.ArtifactRecord record, Set<ModelKey<?>> keys, NumberFormat numberFormat,
