...
public class CSVExporter extends WriterExporterHelper {
  ...
  protected void writeData(PrintWriter out, ExportedData data, ExportParameters parameters) {
    ...
    for (ExportedData.ArtifactRecord record : records) {
      ...
      for (ModelKey<?> key : keys) {
        ...
        Pair<String, ExportValueType> p = key.formatForExport(values, numberFormat, dateFormat, false);
  ...
  private void writeHeader(PrintWriter out, Set<ModelKey<?>> keys, char delimiter) {
    StringBuffer buf = new StringBuffer();
    for (ModelKey<?> key : keys) {
      String name = key.getDisplayableName();
      if (buf.length() > 0)
