...
public class NavigationTreeStringTransfer extends TreeStringTransfer {
  ...
  public NavigationTreeStringTransfer(ATree<ATreeNode<GenericNode>> tree) {
    super(new NavigationTransferService());
    myTree = tree;
  }
  ...
  public boolean canImportData(DragContext context) throws CantPerformException {
    ...
    if (wrappers != null && !wrappers.isEmpty())
      return true;
    MultiMap<?, ?> dropUrls = getDropUrls(context);
  ...
  public boolean canImportDataNow(DragContext context, Component dropTarget) throws CantPerformException {
    ...
    MultiMap<?, ?> dropUrls = haveWrappers ? null : getDropUrls(context);
    ...
    if (haveWrappers || haveUrls) {
      ...
      if (!(userObject instanceof GenericNode))
        return false;
      if (haveWrappers)
        return ((GenericNode) userObject).canAcceptItems(wrappers, context);
      if (haveUrls)
  ...
  private MultiMap<Connection, String> getDropUrls(DragContext context) throws CantPerformException {
    MultiMap<Connection, String> r = context.getValue(CACHED_DROP_URLS);
    if (r != null)
      return r;
    r = extractDropUrls(context);
    if (r == null)
      r = MultiMap.empty();
  ...
  private MultiMap<Connection, String> extractDropUrls(DragContext context) throws CantPerformException {
    ...
    throws CantPerformException, UnsupportedFlavorException, IOException
  {
    ...
    MultiMap<Connection, String> dropUrls = getDropUrls(context);
    if (dropUrls != null && !dropUrls.isEmpty()) {
      ...
      public void loadingDone() {
        ...
        try {
          ...
          if (!list.isEmpty()) {
            ...
            try {
              boolean can = node.canAcceptItems((List) list, acceptContext);
              if (can) {
                node.acceptItems((List) list, acceptContext);
              }
  ...
  private void acceptWrappers(DragContext context, Transferable transfer) throws CantPerformException {
    ...
    if (wrappers != null && wrappers.size() > 0) {
      ...
      Object userObject = targetNode.getUserObject();
      if (userObject instanceof GenericNode) {
        ((GenericNode) userObject).acceptItems(wrappers, context);
      }
    }
  }
