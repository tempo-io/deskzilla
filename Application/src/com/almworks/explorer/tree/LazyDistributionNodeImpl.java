  ...
  implements LazyDistributionExpanderNode, TreeExpansionAware
{
  ...
  public LazyDistributionNodeImpl(String name, Configuration config) {
    super(new FixedText(name, Icons.NODE_DISTRIBUTION_FOLDER_OPEN, Icons.NODE_DISTRIBUTION_FOLDER_CLOSED), config);
    beRemovable();
    addChildNode(new DistributionFolderNodeImpl.ExpandingProgressNode(config));
  }
  ...
  public void expand() {
    ...
    DistributionFolderNodeImpl folder = new DistributionFolderNodeImpl(getPresentation().getText(), config);
