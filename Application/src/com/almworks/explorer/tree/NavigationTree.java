...
public class NavigationTree {
  ...
  public NavigationTree(ComponentContainer container, Configuration config, EventRouter router,
...
//      connectionNode.addChildNode(detachedNote(4, "Use drag-and-drop to move and copy queries."));
  ...
  private NoteNode detachedNote(int order, String name) {
    return new NoteNodeImpl(name, MapMedium.createConfig()).setOrder(order);
  }
  ...
  public void setupTree() {
    ...
    myTree.setTransfer(new NavigationTreeStringTransfer(myTree));
    setupLargeModel();
...
//    });
  ...
  private void setupLargeModel() {
    ...
    TreeModelBridge<GenericNode> sampleNode =
      new TreeModelBridge<GenericNode>(new FolderNode("XXXXXXX", Configuration.EMPTY_CONFIGURATION));
    Component component =
...
//        }
  ...
  private void maybeCreateConnectionLoadingNode(Connection connection) {
    ...
    myRoot.addChildNode(new ConnectionLoadingNodeImpl(myRoot.getConfiguration(), connection));
  }
  ...
  public void start(final ATree<ATreeNode<GenericNode>> tree) {
    ...
    myNodeFactory = new TreeNodeFactoryImpl(tree);
    NodeIdUtil.setConfig(myConfig.getOrCreateSubset("nodeidfactory"));
    myRoot = GenericNodeImpl.createRootNode(myContainer, "Root", myConfig.getOrCreateSubset("Root"), mySyncRegistry,
      myNodeFactory);
    myEventRouter.addListener(ThreadGate.AWT_QUEUED, new EngineListener.Adapter() {
    ...
    Database.require().addListener(Lifespan.FOREVER, new DBListener() {
