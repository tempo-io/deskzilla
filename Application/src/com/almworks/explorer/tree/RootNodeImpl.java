...
class RootNodeImpl extends GenericNodeImpl implements RootNode {
  ...
  public RootNodeImpl(ComponentContainer container, String text, Configuration config, SyncRegistry syncRegistry,
    TreeNodeFactoryImpl nodeFactory)
  {
    super(FixedText.folder(text, Icons.NODE_FOLDER_OPEN, Icons.NODE_FOLDER_CLOSED), config);
    myContainer = container;
    mySyncRegistry = syncRegistry;
    myNodeFactory = nodeFactory;
//    myRecountManager = recountManager;
    myItemsPreviewManager = new ItemsPreviewManager();
...
//    }
  ...
  private TagsFolderNode ensureTagsNodeExists() {
    ...
    if (index < 0) {
      ...
      node = TreeNodeFactoryImpl.TAGS_FOLDER_TYPE.create(subconfig);
  ...
  protected void onFirstInsertToModel() {
    ...
    myRootView = Database.require().filter(BoolExpr.<DP>TRUE());
    listenResolver();
  }
