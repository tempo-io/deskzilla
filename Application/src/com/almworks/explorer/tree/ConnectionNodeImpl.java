...
class ConnectionNodeImpl extends GenericNodeImpl implements ConnectionNode {
  ...
  public ConnectionNodeImpl(Connection connection, Engine engine, Configuration config,
    Map<GenericNode, Integer> orderMap)
  {
    super(
      createPresentation(
    ...
    addChildNode(new TemporaryQueriesNode());
  }
  ...
  private void createSystemNodes() {
    ...
    if (remoteQueries != null) {
      addChildNode(new RemoteQueriesNode(this, remoteQueries));
    }
    ScalarModel<Collection<RemoteQuery2>> remoteQueries2 = myConnection.getRemoteQueries2();
    if (remoteQueries2 != null) {
      addChildNode(
        new RemoteQueries2Node(remoteQueries2, getConfiguration().getOrCreateSubset(REMOTE_QUERIES_2_CONFIG)));
    }
    ConnectionViews views = myConnection.getViews();
    if (myConnection.hasCapability(Connection.Capability.CREATE_ITEM) ||
      myConnection.hasCapability(Connection.Capability.EDIT_ITEM))
    {
      myOutboxNode = new OutboxNode(getNodeId(), views.getOutbox());
  ...
  private void updateIcon() {
    ...
    if (initializationState != null && initializationState.isInitializing()) {
      setIcon(Icons.NODE_CONNECTION_INITIALIZING.getIcon(), "Connection is initializing", true);
    } else if(initializationState == null || !initializationState.isInitialized()) {
      setIcon(Icons.NODE_CONNECTION_WITH_ALERT, "Connection is not initialized", false);
    } else {
      ...
      if (syncState == SyncTask.State.NEVER_HAPPENED)
        setIcon(Icons.NODE_CONNECTION_WITH_ALERT, "Connection was never synchronized", false);
      else if (syncState == SyncTask.State.FAILED)
        setIcon(Icons.NODE_CONNECTION_WITH_ALERT, "Connection synchronization has failed", false);
      else if (syncState == SyncTask.State.SUSPENDED)
        setIcon(Icons.NODE_CONNECTION_WITH_ALERT, "Connection synchronization has been suspended", false);
      else
        setIcon(Icons.NODE_CONNECTION, null, false);
    }
    if (getConnection().isOffline()) {
      setIcon(Icons.NODE_CONNECTION_WITH_ALERT, "Connection is offline, synchronization is disabled", false);
    } else if(getConnection().getAutoSyncMode().getValue() != Connection.AutoSyncMode.AUTOMATIC) {
      setIcon(OFFLINE_CONNECTION_ICON, "Automatic synchronization is disabled", false);
    }
  }
