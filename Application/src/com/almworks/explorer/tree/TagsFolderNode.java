...
public class TagsFolderNode extends GenericNodeImpl {
  ...
  public TagsFolderNode(Configuration config) {
    super(new FixedText("Tags", Icons.NODE_SYSTEM_FOLDER_OPEN, Icons.NODE_SYSTEM_FOLDER_CLOSED), config);
    addAllowedChildType(TreeNodeFactory.NodeType.TAG);
    addAllowedChildType(TreeNodeFactory.NodeType.FAVORITES_TAG);

    // create favorites
    createFixedTagNode(TreeNodeFactory.KLUDGE_FAVORITES, TreeNodeFactoryImpl.FAVORITES_TYPE, FavoritesNode.class, 0);
  }

  private TagNode createFixedTagNode(String subsetName, FolderType<? extends TagNode> folderType,
    Class<? extends TagNode> nodeClass, int insertIndex)
  {
    ...
    if (folder == null) {
      ...
      folder = folderType.create(subconfig);
  ...
  private void deleteTagsNotHavingNode() {
    ...
    Database.require().writeForeground(new WriteTransaction<Object>() {
