...
abstract class GenericNodeImpl implements CanvasRenderable, GenericNode, ModelAware, Getter<Integer> {
  ...
  public GenericNodeImpl(CanvasRenderable presentation, Configuration config) {
    myConfiguration = config;
    myTreeNode = new TreeModelBridge<GenericNode>(this);
    myLoaded = false;
    FolderTypes.loadNodes(this, config);
    myLoaded = true;
  ...
  public static RootNode createRootNode(ComponentContainer container, String text, Configuration config,
    SyncRegistry syncRegistry, TreeNodeFactoryImpl nodeFactory)
  {
    RootNodeImpl node = new RootNodeImpl(container, text, config, syncRegistry, nodeFactory);
    return node;
  }
...
*/
  ...
  public final NameResolver getResolver() {
    return getContainerActor(NameResolver.ROLE);
  }
