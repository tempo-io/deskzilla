...
class RemoteQueriesNode extends GenericNodeImpl implements CollectionModel.Consumer<RemoteQuery> {
  ...
  public RemoteQueriesNode(ConnectionNodeImpl connectionNode, @NotNull CollectionModel<RemoteQuery> remoteQueriesModel) {
    super(FixedText.folder(NAME, Icons.NODE_FOLDER_OPEN, Icons.NODE_FOLDER_CLOSED),
  ...
  public void onScalarsAdded(CollectionModelEvent<RemoteQuery> event) {
    ...
    for (int i = 0; i < event.size(); i++) {
      ...
      addChildNode(new RemoteQueryNode(myConnectionNode.getNodeId(), remoteQuery));
    }
  }
