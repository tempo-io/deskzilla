...
public class TagEditor {
  ...
  private static final IconData NO_ICON = new IconData("no icon", PresentationMapping.EMPTY_ICON, TagsUtil.NO_ICON);
  ...
  private void rescan() {
    ...
    if (workArea != null) {
      Collection<File> files = workArea.getEtcCollectionFiles(TagsUtil.TAG_ICONS_COLLECTION);
      if (files != null) {
        for (File file : files) {
          String name = file.getName();
          if (isIconFile(name)) {
            Icon icon = new FileCollectionBasedIcon(TagsUtil.TAG_ICONS_COLLECTION, name, true);
