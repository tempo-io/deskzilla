...
class FolderTypes {
  ...
  public static void loadNodes(GenericNode parent, Configuration config) {
    ...
    for (Configuration configuration : nodeConfigs) {
      GenericNode node = loadNode(configuration);
      if (node != null) insert.invoke(node);
    }
  }

  @Nullable
  public static GenericNode loadNode(Configuration configuration) {
    FolderType nodeType = getNodeType(configuration);
    return nodeType != null ? nodeType.create(configuration) : null;
  }
