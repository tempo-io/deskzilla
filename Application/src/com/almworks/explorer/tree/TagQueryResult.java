...
public class TagQueryResult extends AbstractQueryResult {
  private static final String FAVORITES_TAG_ITEM_ID = TagsComponent.NS.obj(TreeNodeFactoryImpl.FAVORITES_TAG_NODE_ID);
  ...
  public TagQueryResult(TagNodeImpl node) {
    ...
    myFilter = DPIntersectsIdentified.create(TagsComponent.TAGS, Collections.singleton(myTag));
  }
  ...
  public ItemHypercube getHypercube(boolean precise) {
    return myNode.getHypercube(precise);
  }
  ...
  public void delete() {
    if (!isInitialized())
      return;
    final BoolExpr<DP> taggedItems = myView.get().getExpr();
    // Not using ItemSync here: tags are not uploadable
    Database.require().writeBackground(new WriteTransaction<Object>() {
  ...
  private void doUpdateTagName(Procedure<Long> awtInitializer) {
    ...
    DBResult<Long> res = Database.require().writeForeground(new WriteTransaction<Long>() {
      public Long transaction(DBWriter writer) {
        ...
        TagsComponent.ICON_PATH.setValue(writer, tagItem, iconPath);
  ...
  static String getTagId(GenericNode node) {
    return TagsComponent.NS.obj(node.getNodeId());
  }

  public static class TagItem extends DBIdentifiedObject {
    public TagItem(String id, String name) {
      super(id);
      initialize(DBAttribute.TYPE, TagsComponent.TYPE_TAG);
  ...
  private class MyLazyView extends Lazy<DBFilter> {
    ...
    protected DBFilter instantiate() {
      ...
      return view == null ? new DBFilter(Database.require(), myFilter) : view;
    }
