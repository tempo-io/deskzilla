...
public class ExplorerComponentImpl implements Startable, ExplorerComponent {
  ...
  public ExplorerComponentImpl(MainWindowManager windowManager, Configuration configuration,
    ...
    ActionRegistry actionRegistry, WorkflowComponent2 workflow, ApplicationToolbar toolbar,
    SyncRegistry syncRegistry, WorkArea workArea)
  {
    myWorkArea = workArea;
...
//      assert false : item;
  ...
  public void start() {
    ThreadGate.AWT_IMMEDIATE.execute(new Runnable() {
      public void run() {
        ...
        myExplorer = new Explorer(myConfiguration.getOrCreateSubset("explorer"),
          myEngine.getConnectionManager().getProviders(), myProductInfo, myCurrentSelection, myWorkflow, myWorkArea, appStatus,
          myContainer.getActor(ColumnsCollector.ROLE), ExplorerComponentImpl.this, myEngine);
      }
    });
    registerActions();
  ...
  private void registerActions() {
    myActionRegistry.registerAction(MainMenu.File.NEW_CONNECTION, new SimpleAction() {
      ...
      private ItemProvider getTheProvider() {
        ...
        for(final ItemProvider p : providers) {
          if(p.isEnabled()) {
            return p;
          }
        }
  ...
  public ScalarModel<Boolean> isNavigationTreeReady() {
    return myTree.isReady();
  }
  ...
  public ItemsCollectionController createLoader(ItemCollectorWidget widget, ItemSource source) {
    return new ItemsCollectorImpl(myEditRegistry, widget, source);
  }
