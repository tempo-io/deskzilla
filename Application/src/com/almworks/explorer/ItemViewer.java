...
public class ItemViewer {
  ...
  private void setupDelayedFullDownload(Lifespan lifespan, final LoadedItem item) {
    ...
    if (stage != ItemDownloadStage.FULL) {
      ...
      if (delay > 0 && connection != null) {
        Context.require(TimeService.ROLE).awtInvokeIn(lifespan, delay * 1000, new Computable<Object>() {
          ...
          public Object compute() {
            connection.downloadItemDetails(LongArray.singleton(item.getItem()));
            return null;
          }
        });
      }
  ...
  private JComponent createSmartToolbar(ToolbarBuilder builder) {
    final ItemUrlService urlService = Context.get(ItemUrlService.class);
    if(urlService == null) {
