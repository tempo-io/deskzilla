...
class TableControllerImpl implements ItemCollectorWidget, TableController {
  ...
  private final Bottleneck myUpdateFilteringModelBottleneck = new Bottleneck(200, ThreadGate.AWT, new Runnable() {
    ...
    public void run() {
      boolean smthFound =
        onSearchFilterChanged(myTitle.getHighlightText(), myTitle.getRegexp(), myTitle.getCaseSensitive(),
  ...
  public TableControllerImpl(Configuration config, ContentTab tab, WorkflowComponent2 workflow,
    ColumnsCollector columnsCollector, ExplorerComponent explorer)
  {
    myExplorer = explorer;
    myConfig = config;
    myColumnsCollector = columnsCollector;
    myOutOfDateStripe.showStripe(false);
    myHierarchy = new HierarchyController(myDatas, myArtifactsTree, myMetaInfoCollector);
...
//    myArtifactsTree.setSortPolicy(CollectionSortPolicy.QUICK);
    ...
    setupTable(myArtifactsTree, workflow, myScrollpane);
...
//    myScrollpane.getViewport().setScrollMode(JViewport.BACKINGSTORE_SCROLL_MODE);
  ...
  private boolean onSearchFilterChanged(String str, boolean regexp, boolean caseSensitive, boolean filter) {
  ...
  private void setupTable(final HierarchicalTable<LoadedItem> table, WorkflowComponent2 workflow,
    JScrollPane scrollPane)
  {
    ...
    builder.addSeparator()
      .addEnabledActionsFromModel(workflow.getWorkflowActions())
      .addSeparator()
      .addAction(StdItemActions.AUTO_ASSIGN)
      .addAction(StdItemActions.ADD_COMMENT)
  ...
  public List<TableColumnAccessor<LoadedItem, ?>> getSelectedColumns() {
    return Collections15.arrayList(myArtifactsTree.getHeaderController().getUserColumnsSubsetModel().toList());
  }
  ...
  private void createCopy(ContentTab tab, WorkflowComponent2 workflow) {
    ...
    assert contextInfo != null;
    TableControllerImpl copy = new TableControllerImpl(myConfig, tab, workflow, myColumnsCollector, myExplorer);
    ...
    LongList selection = LongSet.collect(UiItem.GET_ITEM, getSelectedArtifacts().getSelectedItems());
  ...
  private void matchesPatternSwitch(ModelKey key, PropertyMap map, StringBuilder buf) {
    if (key instanceof BaseModelKey) {
      String str = ((BaseModelKey) key).matchesPatterString(map);
      if (str != null) {
    ...
    if (val != null) {
      ...
      if (val instanceof ItemKey) {
        ...
        for (Object v : (Collection) val) {
          if (v instanceof BaseModelKey) {
            matchesPatternSwitch((BaseModelKey) v, map, buf);
          } else if (v instanceof Comment) {
            buf.append(((Comment) v).getText()).append(' ');
          } else if (v instanceof Attachment) {
            buf.append(((Attachment) v).toSearchableString());
