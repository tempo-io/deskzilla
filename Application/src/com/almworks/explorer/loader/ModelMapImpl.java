...
public class ModelMapImpl extends SimpleModifiable implements ModelMap {
  ...
  private final Map<TypedKey<?>, Object> myEditServices = Collections15.hashMap();
  ...
  public Collection<? extends ModelKey> getAllKeys() {
  ...
  public void mergeFrom(PropertyMap baseValues, PropertyMap newValues) {
    assert checkSameKeys(baseValues);
    assert checkSameKeys(newValues);
    myChangeCounter = 0;
    try {
      BitSet2 allKeys = ModelKey.ALL_KEYS.getValue(newValues);
      for (int i = allKeys.nextSetBit(0); i >= 0; i = allKeys.nextSetBit(i + 1)) {
        ModelKey<?> key = ModelKeySetUtil.getKey(i);
        if (key != null) {
          key.getMergePolicy().mergeIntoModel(key, this, baseValues, newValues);
        }
      }
      if (myChangeCounter > 0) {
        fireChanged();
  ...
  public <T> void putService(TypedKey<T> key, T service) {
    key.putTo(myEditServices, service);
  }

  @Override
  public <T> T getService(TypedKey<T> key) {
    return key.getFrom(myEditServices);
  }
}
