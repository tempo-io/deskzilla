...
public class LoadedItemImpl extends BaseItemWrapper implements LoadedItem {
  ...
  public boolean matches(@NotNull Constraint constraint) {
    return matches(constraint, getLastDBValues());
  }

  public static boolean matches(Constraint constraint, final PropertyMap lastValues) {
    TypedKey<? extends Constraint> type = constraint.getType();
    if (type == Constraint.TRUE)
      return true;
    final Map<DBAttribute, ModelKey<? extends ItemKey>> keyMap = Collections15.hashMap();
    BitSet2 allKeys = ModelKey.ALL_KEYS.getValue(lastValues);
    for (int i = allKeys.nextSetBit(0); i >= 0; i = allKeys.nextSetBit(i + 1)) {
      ModelKey<?> key = ModelKeySetUtil.getKey(i);
      if (key instanceof AttributeModelKey) {
        keyMap.put(((AttributeModelKey) key).getAttribute(), (ModelKey<? extends ItemKey>) key);
      }
    }
    // todo #854 extract interface
    try {
      return Constraints.substitute(new Constraints.Substitution() {
        public boolean substitute(@NotNull Constraint constraint) throws Constraints.IllegalSubstitutionException {
          FieldEqualsConstraint equalTo = Constraints.cast(FieldEqualsConstraint.EQUALS_TO, constraint);
          if (equalTo == null)
            throw new Constraints.IllegalSubstitutionException(constraint);
          ModelKey<? extends ItemKey> key = keyMap.get(equalTo.getAttribute());
          if (key == null)
            throw new Constraints.IllegalSubstitutionException("Unknown attribute: " + equalTo.getAttribute());
          ItemKey value = key.getValue(lastValues);
          long artifact = value != null ? value.getResolvedItem() : 0;
          return artifact == equalTo.getExpectedValue();
        }
      }, constraint);
    } catch (Constraints.IllegalSubstitutionException e) {
      Log.warn(e);
      return false;
    }
  }
