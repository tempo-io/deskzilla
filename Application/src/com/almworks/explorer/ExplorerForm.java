...
class ExplorerForm {
  ...
  public ExplorerForm(Configuration config, Collection<ItemProvider> providers, ProductInformation productInfo,
    ApplicationLoadStatus appStatus, Engine engine)
  {
    ...
    myNavigation = new ATree<ATreeNode<GenericNode>>();
    ...
    myConfig = config;
    mySumtable =
      new ExplorerDistributionTable(myNavigation.getSelectionAccessor(), myConfig.getOrCreateSubset("sumtable"));
  ...
  private WelcomeScreen getWelcomeScreen() {
    if(myWelcomeOverall == null) {
      myWelcomeOverall = new WelcomeScreen(myProductInfo, true);
      // kludge :(
