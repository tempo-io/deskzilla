...
public abstract class DateUnit {
  ...
  private static void correctDayTime(Calendar calendar, boolean laterBound) {
    calendar.set(Calendar.HOUR_OF_DAY, laterBound ? 23 : 0);
    calendar.set(Calendar.MINUTE, laterBound ? 59 : 0);
    calendar.set(Calendar.SECOND, laterBound ? 59 : 0);
    calendar.set(Calendar.MILLISECOND, laterBound ? 999 : 0);
  }
  ...
  public static abstract class DateValue {
    ...
    public boolean before(@NotNull Date date) {
      ...
      return Comparing.isSameDay(value, date) || value.before(date);
    }
    ...
    public boolean after(@NotNull Date date) {
      Date value = getUptoDateValue();
      return Comparing.isSameDay(value, date) || value.after(date);
    }
  ...
  public static class RelativeDate extends DateValue {
    ...
    public BoolExpr<DP> afterExpr(DBAttribute<Date> attr) {
      assert !myLaterBound;
      return new RelativeDateDP(attr).term();
    }
    ...
    private class RelativeDateDP extends DP {
      private final DBAttribute<Date> myAttr;

      private RelativeDateDP(DBAttribute<Date> attr) {
        myAttr = attr;
      }
...
//        updateSubscriptionViaSwingTimer(subscription, subscriptionLife, nextUpdateTime);
  ...
  public static class AbsoluteDate extends DateValue {
    ...
    @Override
    public BoolExpr<DP> afterExpr(DBAttribute<Date> attr) {
      // todo :use: DPAbsoluteDate which reresolves when time zone is changed
