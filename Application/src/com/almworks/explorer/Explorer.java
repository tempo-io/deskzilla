...
class Explorer {
  ...
  public Explorer(Configuration configuration, Collection<ItemProvider> providers, ProductInformation productInfo,
    final ValueModel<SelectionAccessor<LoadedItem>> selectionModelModel, WorkflowComponent2 workflow,
    WorkArea workArea, ApplicationLoadStatus appStatus, ColumnsCollector columnsCollector, ExplorerComponent explorer,
    Engine engine)
  {
    myExplorer = explorer;
    ...
    myWorkflow = workflow;
    myForm = new ExplorerForm(myFormConfig, providers, productInfo, appStatus, engine);
  ...
  SearchResult setCollectionContext(final ItemSource source, ItemCollectionContext contextInfo, boolean focusToTable) {
    ...
    TableControllerImpl tableController =
      new TableControllerImpl(myFormConfig, tab, myWorkflow, myColumnsCollector, myExplorer);
