...
public class OpenInNewTabAction extends SimpleAction {
  ...
  protected void doPerform(final ActionContext context) throws CantPerformException {
    ...
    if (items.size() == 1) {
      ...
      showMultipleArtifacts(explorer, items);
    }
  }

  private void showMultipleArtifacts(final ExplorerComponent explorer, final List<ItemWrapper> items) throws CantPerformException {
    Database.require().readForeground(new ReadTransaction<Pair<String, String>>() {
      @Override
  ...
  private void showTab(String contextName, String tooltip, List<ItemWrapper> items, ExplorerComponent explorer) {
    ...
    CollectionBasedItemSource source = CollectionBasedItemSource.create(items);
