...
class SummaryTable {
  ...
  public void attach(Lifespan lifespan, final GenericNode node, QueryResult result) {
    ...
    myCounter.attach(lifespan, node);
  ...
  private void updateTable() {
    ...
    if (table != null) {
      if (!table.isDataAvailable()) {
        ...
        int colSorterIndex = colSorter == null ? -1 : columns.indexOf(colSorter);
        ...
        int rowSorterIndex = rowSorter == null ? -1 : rows.indexOf(rowSorter);
  ...
  private static class GridHeaderRenderer extends BaseRendererComponent implements CollectionRenderer<STFilter> {
    ...
    private void paintHeader(Graphics g) {
      ...
      if (Util.equals(myItem, myConfiguration.getSortFilter())) {
