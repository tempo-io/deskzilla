...
public class ImportTagsOnFirstRun {
  ...
  private boolean exportTagsFromOldDb(Lifespan lifespan, Progress progress) throws Exception {
    ...
    Class<?> tagExporterClass = jarLoader.loadClass("com.almworks.tools.tagexporter.TagExporter");
    Method exportTagsMethod = tagExporterClass.getMethod("exportTags", File.class, File.class, Progress.class, Procedure2.class);
    Method checkIsOldDb = tagExporterClass.getMethod("is1xWorkspace", File.class);
    ...
    setupLoggingProgress(lifespan, progress);
  ...
  {
    setupLoggingProgress(life, progress);
    ...
    Procedure<String> onFatalError = new Procedure<String>() {
      @Override
  ...
  private static Progress setupLoggingProgress(Lifespan life, final Progress p) {
    p.getModifiable().addChangeListener(life, ThreadGate.STRAIGHT, new ChangeListener() {
      @Override
      public void onChange() {
