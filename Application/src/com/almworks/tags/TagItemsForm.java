...
class TagItemsForm implements UIComponentWrapper {
  ...
  public TagItemsForm() {
    ...
    SpeedSearchController.install(myTable, 0, TAG_RENDERER).setIgnoreSpace(true);
  ...
  public void attach(List<ResolvedTag> commonTags, Collection<TagNode> tagNodes) {
  ...
  private boolean isSet(List<ResolvedTag> commonTags, TagNode tagNode) {
    if (commonTags.isEmpty())
      return false;
    long item = tagNode.getTagItem();
    if (item == 0) return false;
    for (ResolvedTag commonTag : commonTags) {
