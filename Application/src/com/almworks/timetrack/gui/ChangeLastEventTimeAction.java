...
class ChangeLastEventTimeAction extends SimpleAction {
  ...
  protected void customUpdate(UpdateContext context) throws CantPerformException {
    ...
    if (time != 0) {
      ...
      final long nowDay = DateUtil.truncDay(System.currentTimeMillis(), tz);
      final long thenDay = DateUtil.truncDay(time, tz);
  ...
  protected void doPerform(final ActionContext context) throws CantPerformException {
    ...
    final ItemModelRegistry modelRegistry = Context.get(ItemModelRegistry.class);
    ...
    Database.require().readForeground(new ReadTransaction<List<TimePeriod>>() {
      @Override
      public List<TimePeriod> transaction(DBReader reader) throws DBOperationCancelledException {
        return loadPeriods(records, modelRegistry, ttc, currTask, reader);
      }
  ...
  private List<TimePeriod> loadPeriods(Map<TimeTrackerTask, List<TaskTiming>> records,
    ItemModelRegistry modelRegistry, TimeTrackingCustomizer ttc, TimeTrackerTask currTask, DBReader reader)
  {
    ...
    for(final Map.Entry<TimeTrackerTask, List<TaskTiming>> e : records.entrySet()) {
      ...
      final LoadedItem loadedItem = task.load(modelRegistry, reader);
      if(loadedItem != null) {
        final String id = ttc.getItemKey(loadedItem);
