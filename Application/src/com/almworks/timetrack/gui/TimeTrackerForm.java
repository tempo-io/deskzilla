...
public class TimeTrackerForm implements UIComponentWrapper {
  ...
  private void updateIssue(TimeTrackerTask task, TimeTrackerTask lastTask) {
    ...
    if (myShownTask == null) {
      ...
      Database.require().readForeground(new ReadTransaction<LoadedItem>() {
        @Override
  ...
  private void attachIssue(Lifespan life, LoadedItem item) {
    if (life.isEnded() || item.services().isDeleted()) {
      return;
    }
