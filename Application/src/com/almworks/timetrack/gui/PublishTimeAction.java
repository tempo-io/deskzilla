...
import com.almworks.timetrack.gui.timesheet.*;
...
public class PublishTimeAction extends SimpleAction {
  ...
  protected void customUpdate(UpdateContext context) throws CantPerformException {
    ...
    if (!DBDataRoles.isAnyConnectionHasAnyCapability(context, Connection.Capability.EDIT_ITEM)) {
  ...
  protected void doPerform(ActionContext context) throws CantPerformException {
    ...
    Database.require().readForeground(new ReadTransaction<Map<TimeTrackerTask, LoadedItem>>() {
      @Override
...
//    builder.setIgnoreStoredSize(true);
    ...
    JCheckBox checkbox =
      CommentEditorDialog.createCommitImmediatelyCheckbox(builder.getConfiguration(), commitImmediately);
  ...
  private AnAction createPublishAction(final SynchronizedBoolean flag, final TimesheetFormData data) {
    return new SimpleAction("&Publish") {
      protected void doPerform(ActionContext context) throws CantPerformException {
        ...
        final EditCommit commit = ttc.createPublishingCommit(timeMap, remMap, deltas, flag.get());
        if(commit != null) {
          syncMan.unsafeCommitEdit(commit);
        }
