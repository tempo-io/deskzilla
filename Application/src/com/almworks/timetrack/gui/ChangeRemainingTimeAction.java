...
public class ChangeRemainingTimeAction extends SimpleAction {
  ...
  private void listenToArtifact(UpdateContext context, final TrackerSimplifier ts) {
    final UpdateRequest ur = context.getUpdateRequest();
    Database.require().addListener(ur.getLifespan(), new DBListener() {
  ...
  private void loadRemainingTime(final TimeTrackerTask task) {
    Database.require().readForeground(new ReadTransaction<TaskRemainingTime>() {
      @Override
      public TaskRemainingTime transaction(DBReader reader) throws DBOperationCancelledException {
        final LoadedItem loaded = task.load(reader);
        if(loaded != null) {
          final Integer savedRemaining = myCustomizer.getRemainingTime(loaded);
