...
public class TimesheetFormData {
  ...
  private final List<WorkPeriod> myWorkList = Collections15.arrayList();
  private final List<WorkPeriod> myWorkUnmodifiable = Collections.unmodifiableList(myWorkList);
      ...
      for (final Map.Entry<LoadedItem, List<TaskTiming>> e : timeMap.entrySet()) {
        ...
        if (item != null && timings != null) {
          for (TaskTiming timing : timings) {
            if (TimesheetForm.okForTiming(timing.getStarted(), timing.getStopped())) {
              WorkPeriod period = new WorkPeriod(timing, item);
              myWorkList.add(period);
  ...
  public List<WorkPeriod> getWorkList() {
    return myWorkUnmodifiable;
  }
  ...
  private void insertPeriodSorted(WorkPeriod period) {
    int k = Collections.binarySearch(myWorkList, period);
    if (k < 0) {
      k = -k - 1;
    }

    myWorkList.add(k, period);
  }
