...
public class UploadOnSuccess implements EditCommit {
  ...
  public static UploadOnSuccess create(long item) {
    return create(LongArray.singleton(item));
  }

  public static UploadOnSuccess createForMaster(long slave, DBAttribute<Long> masterAttribute) {
    return new UploadOnSuccess(LongArray.singleton(slave), masterAttribute, Context.require(Engine.ROLE));
  }
  ...
  public static EditCommit create(Collection<? extends UiItem> items) {
    return create(LongSet.collect(UiItem.GET_ITEM, items));
  }
  ...
  public void performCommit(EditDrain drain) throws DBOperationCancelledException {
    if(myMasterAttribute != null) {
      final LongSet masters = new LongSet();
