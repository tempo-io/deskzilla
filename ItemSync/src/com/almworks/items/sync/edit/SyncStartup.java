...
class SyncStartup {
  ...
  public <T> DBResult<T> readForeground(ReadTransaction<T> transaction) {
    return myDatabase.readForeground(transaction);
  }
  ...
  private void clearIllegalState(DBWriter writer) {
    ...
    illegals.addAll(writer.query(DPNotNull.create(SyncSchema.DONE_UPLOAD)).copyItemsSorted());
    ...
    if (!illegals.isEmpty()) {
      ...
      for (int i = 0; i < illegals.size(); i++) {
        ...
        boolean doneUpload = writer.getValue(item, SyncSchema.DONE_UPLOAD) != null;
  ...
  private void finishUploads(DBWriter writer) {
    LongArray items = writer.query(DPNotNull.create(SyncSchema.DONE_UPLOAD)).copyItemsSorted();
  ...
  private void clearUploads(DBWriter writer) {
    ...
    for (int i = 0; i < failedUploads.size(); i++) {
      ...
      if (writer.getValue(item, SyncSchema.BASE) != null) {
        writer.setValue(item, SyncSchema.UPLOAD_FAILED, true);
