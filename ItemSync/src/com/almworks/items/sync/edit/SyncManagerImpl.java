...
public class SyncManagerImpl implements SyncManager {
  ...
  public Lifespan upload(long item, ItemUploader uploader) {
    return upload(LongArray.create(item), uploader);
  }

  @Override
  public Lifespan upload(LongList items, ItemUploader uploader) {
    return new UploadTask(this, uploader, items).start();
  }

  @Override
  public boolean canUpload(long item) {
    return !getUploadLocks().isAnyLocked(LongArray.singleton(item));
  }
   ...
   * @param itemsToEnqueue if any of itemsToLock can not be locked right now enqueue itemToLock to merge later
   * @return true iff the itemToLock is locked for merge, false means that the item can not be locked and itemToEnqueue is to be merged later
   */
  ...
  LongList selectMergableNowDeferOther(DBWriter transaction, LongList items, @Nullable EditorLock ignoreLock) {
    ...
    for (int i = 0; i < items.size(); i++) {
      ...
      LongArray asList = LongArray.singleton(item);
  ...
  private void addToStart(@Nullable Collection<EditStart> starts) {
    ...
    if (!empty) {
      ...
      if (myStartTransaction.compareAndSet(null, readTransaction)) myDatabase.readForeground(readTransaction);
    }
  }
