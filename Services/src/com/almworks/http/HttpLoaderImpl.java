...
public class HttpLoaderImpl implements HttpLoader {
  ...
  public void setMaximumRedirects(int maximumRedirects) {
    myMaximumRedirects = maximumRedirects;
  }
...
//    assert myRetries >= 3;
    ...
    try {
      ...
      for (int i = 0; i < myRetries; i++) {
        ...
        try {
          ...
          AuthResult auth = handleAuthRequest(executor, statusCode);
    ...
    throws AbortLoading, IOException, HttpCancelledException
  {
    ...
    if (shouldAskForCredentials(authState, executor)) {
      boolean credentialsChanged = loadCredentials(executor, true);
      result = credentialsChanged ? AuthResult.AVAILABLE_NEW : AuthResult.AVAILABLE_THESAME;
    }
    return result;
  }
    ...
    throws IOException, AbortLoading, HttpCancelledException
  {
    ...
    if (shouldAskForCredentials(authState, executor)) {
      boolean credentialsChanged = loadCredentials(executor, false);
      result = credentialsChanged ? AuthResult.AVAILABLE_NEW : AuthResult.AVAILABLE_THESAME;
    }
    return result;
  }
  ...
  private HttpResponseData handleRedirect(HttpMethodExecutor executor, int statusCode)
    throws HttpLoaderException, HttpConnectionException, IOException, HttpMethodFactoryException
  {
    ...
    URI redirectURI;
    try {
      redirectURI = adjustRedirectURI(new URI(redirectUrl, true), currentURI);
    } catch (URIException e) {
    ...
    if (redirectHandlers != null) {
      for (RedirectURIHandler redirectHandler : redirectHandlers) {
        URI converted = redirectHandler.approveRedirect(currentURI, redirectURI);
  ...
  private static URI adjustRedirectURI(URI redirectURI, URI currentURI) throws URIException {
    ...
    if (query.length() == 0) {
      if (Util.NN(currentURI.getEscapedQuery()).length() > 0) {
  ...
  private HttpResponseData createReply(HttpMethodExecutor executor) throws IOException {
    ...
    if (content == null) {
      Log.warn("got null input stream", new IOException("got null input stream"));
      return null;
    }
    ...
    InputStream decodedContent = decodeContent(content, executor);
    if (decodedContent == null) {
      reply.setContentStream(content);
      reply.setContentLength(executor.getResponseContentLength());
    } else {
      reply.setContentStream(decodedContent);
  ...
  @Nullable
  private static InputStream decodeContent(InputStream content, HttpMethodExecutor executor) throws IOException {
  ...
  private boolean loadCredentials(HttpMethodExecutor executor, boolean proxy)
    throws IOException, AbortLoading, HttpCancelledException
  {
  ...
  private void setResponseCharset(HttpResponseDataImpl reply, HttpMethodExecutor executor) {
    ...
    if (charset == null || charset.length() == 0)
      charset = executor.getResponseCharSet();
