...
import static com.almworks.tools.tagexporter.TagExporter.*;
...
public class TagExporterForm {
  ...
  public static void showWindow() {
    JFrame frame = new JFrame(L.frame(TOOL_NAME));
    ...
    TagExporterForm form = new TagExporterForm();
    form.readProductId();
    frame.setTitle(form.myAppTitleName + " 1.x\u20132.x Tag Exporter");
  ...
  private void readProductId() {
    ...
    if(PRODUCT_ID_JIRACLIENT.equals(myProductId)) {
      myAppLabelName = myAppTitleName = "JIRA Client";
    } else if(PRODUCT_ID_DESKZILLA.equals(myProductId)) {
      myAppLabelName = myAppTitleName = "Deskzilla";
    } else {
      myAppTitleName = "Deskzilla and JIRA Client";
      myAppLabelName = "Deskzilla or JIRA Client";
    }
  }
  ...
  private boolean verifyWorkspace(File workspace) {
    String error = ArtifactDb.verifyWorkspace(workspace);
    setError(WORKSPACE_KEY, error);
    return error == null;
  }
  ...
  private void setCanonicalOutputFile(String outFile) {
    try {
      ...
      setError(OUT_FILE_KEY, e.getMessage());
    }
  }

  private void updateStatus() {
    String message = myErrors.get(GLOBAL_KEY);
    Map.Entry<String, String> localError = first(myErrors.entrySet());
  ...
  private void setError(String key, @Nullable String msg) {
    ...
    if (key != GLOBAL_KEY) myErrors.remove(GLOBAL_KEY);
    updateStatus();
  }
  ...
  private class ExportTagsAction extends SimpleAction {
    ...
    protected void doPerform(ActionContext context) throws CantPerformException {
      ...
      TagExporter tagExporter = exportTags(workArea, outFile, progress, new Procedure2<String, String>() {
        @Override
