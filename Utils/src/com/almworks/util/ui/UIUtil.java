...
public class UIUtil {
   ...
   */
  public static final Border EDITOR_PANEL_BORDER = Aqua.isAqua() ? UIUtil.BORDER_9 : UIUtil.BORDER_5;
  ...
  public static final Procedure<Component> DISABLE_PROCEDURE = new Procedure<Component>() {
...
*/
  ...
  public static <T> T getImplementor(Object element, Class<? extends T> aClass) {
    if (aClass.isInstance(element))
      return Util.cast(aClass, element);
    else
...
//          selection.removeSelectionAt(index);
  ...
  public static void addTextListener(Lifespan life, final JTextComponent component, final ChangeListener listener) {
    life.add(addTextListener(component, listener));
  }

  /**
   * @deprecated replace with {@link #addTextListener(org.almworks.util.detach.Lifespan, javax.swing.text.JTextComponent, com.almworks.util.collections.ChangeListener)}
   */
  @Deprecated
  public static Detach addTextListener(final JTextComponent component, final ChangeListener listener) {
    ...
    final StringChecker stringChecker)
  {
    ...
    result.add(addTextListener(textComponent, new ChangeListener() {
  ...
  public static void centerWindow(Window window) {
    ...
    Rectangle bounds = getScreenUserSize(gconf);
    Dimension size = window.getSize();
    int x = bounds.x + Math.max(0, (bounds.width - size.width) / 2);
...
//    to.setFocusTraversalKeys(KeyboardFocusManager.DOWN_CYCLE_TRAVERSAL_KEYS, from.getFocusTraversalKeys(KeyboardFocusManager.DOWN_CYCLE_TRAVERSAL_KEYS));
  ...
  public static void selectAll(JTextComponent c) {
    ...
    int len = Util.NN(getDocumentText(c)).length();
    ...
    editorPane.addHyperlinkListener(new HyperlinkListener() {
      public void hyperlinkUpdate(HyperlinkEvent e) {
        if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
          URL url = e.getURL();
          ExternalBrowser.openURL(url.toExternalForm(), true);
...
//          }
  ...
  public static Detach showNoticePopup(String htmlText, Component owner, Detach onClose, Positioner positioner) {
    if(owner == null) {
  ...
  public static void setInitialFocus(final Component c) {
    ...
    detach.add(new Detach() {
      protected void doDetach() throws Exception {
        c.removeHierarchyListener(listener);
      }
    });
  }
