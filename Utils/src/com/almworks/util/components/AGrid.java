...
public class AGrid<R, C, V> {
  ...
  private final GridHeader<R> myGridRowHeader = new GridHeader<R>(myRowModel, myRowHeaderRenderer, true);
  private final GridHeader<C> myGridColumnHeader = new GridHeader<C>(myColumnModel, myColumnHeaderRenderer, false);
  ...
  private final Bottleneck myGridLayoutBottleneck = new Bottleneck(1000, ThreadGate.AWT, new Runnable() {
    public void run() {
      layoutGrid();
    }
  });
  ...
  public AGrid() {
    ...
    myModel = new MyTableModelAdapter<R, C, V>(myRowModel, myColumnModel, myCellModel);
    ...
    myTable.setDefaultRenderer(Object.class, new MyCellRenderer<V>(myRenderer));
...
//    myGridLayoutBottleneck.request();
  ...
  private static class MyTableModelAdapter<R, C, V> extends AbstractTableModel {
    ...
    public void attach() {
      myRowModel.addListener(new AListModel.Listener() {
      ...
      myColumnModel.addListener(new AListModel.Listener() {
        public void onInsert(int index, int length) {
          fire();
        }
  ...
  private static class MyCellRenderer<V> extends BaseRendererComponent implements TableCellRenderer {
      ...
      int row, int column)
    {
      ...
      if (renderer == null)
        return this;
      hasFocus = SpeedSearchController.fixFocusedState(table, hasFocus, row, column);
      MyCellState state = new MyCellState(table, isSelected, hasFocus, row, column);
      ((GridTable) table).reportNextState(state);
      JComponent c = renderer.getRendererComponent(state, (V) value);
      return c;
    }
  }
...
//          }
  ...
  private static class MyCellState extends ComponentCellState {
    ...
    private boolean isSelectionDrawn() {
      return mySelected &&  SpeedSearchController.isFocusOwner(getComponent());
    }
