      ...
      if (e instanceof TableModelAdapter.HeaderTableModelEvent) {
        ...
        if (type == TableModelEvent.INSERT) {
          ...
          for (int i = firstIndex; i <= headerEvent.getLastIndex(); i++) {
            TableColumn column = new MyTableColumn(i, this);
            addColumn(column);
...
//    }
  ...
  void setTableModel(TableModelAdapter<T> model) {
    ...
    final int toIndex = columnModel.getColumnCount() - 1;
    updateColumnPreferredWidths(fromIndex, toIndex);
    updateRowHeights();
    updatePreferredScrollableViewportSize();
  }
...
*/
  ...
  public TableCellState getCellState(int row, int column) {
    ...
    boolean hasFocus = (rowIsLead && colIsLead) && SpeedSearchController.isFocusOwner(this);
  ...
  public void createDefaultColumnsFromModel() {
    ...
    if (m != null) {
      ...
      for (int i1 = 0; i1 < m.getColumnCount(); i1++) {
        TableColumn newColumn = new MyTableColumn(i1, this);
        this.addColumn(newColumn);
      }
    }
  }

  public void updateColumnPreferredWidths(final int fromIndex, final int toIndex) {
    TableColumnModel columnModel = getColumnModel();
    AListModel<TableColumnAccessor<? super T, ?>> columns = getColumns();
  ...
  private static class MyTableColumn extends TableColumn {
    ...
    public MyTableColumn(int i, JTableAdapter<?> adapter) {
      super(i);
      myAccessor = adapter.getColumns().getAt(i);
