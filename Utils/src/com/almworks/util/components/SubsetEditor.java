...
public final class SubsetEditor<T> extends UIComponentWrapper2Support {
  ...
  public static <T> SubsetEditor<T> create(final SubsetModel<T> model, boolean showReordering,
    Convertor<T, String> filteringConvertor, @Nullable Function<String, T> creator)
  {
    return new SubsetEditor<T>(model, null, null, null, showReordering, filteringConvertor, creator);
  }
  ...
  private class Form {
    ...
    public void setCanvasRenderer(CanvasRenderer<? super T> renderer) {
      ...
      SpeedSearchController.install(mySourceList);
      SpeedSearchController.install(myTargetList);
    }
