...
public class WidthDrivenColumn extends JComponent implements Scrollable, WidthDrivenComponent {
  ...
  public Dimension getPreferredSize() {
    int maxPrefWidth = getMaxPrefWidth(myComponents);
    int viewportWidth = getViewportWidth();
    if (viewportWidth > 0)
      maxPrefWidth = Math.max(maxPrefWidth, viewportWidth - AwtUtil.getInsetWidth(this));
    int height = getSumPreferedHeight(maxPrefWidth, myComponents, myVericalGap);
    return myArea.toWhole(maxPrefWidth, height);
  }
