...
public class ConfigAccessors {
  ...
  public static PropertyMap readActionPresentation(ReadonlyConfiguration configuration) {
    PropertyMap result = new PropertyMap(null);
    Collection<String> names = configuration.getAllSettingNames();
    for (Iterator<String> iterator = names.iterator(); iterator.hasNext();) {
      String name = iterator.next();
      PresentationKey<?> key = PresentationKey.forName(name);
      if (key != null)
        try {
          key.load(configuration.getMandatorySetting(name), result);
