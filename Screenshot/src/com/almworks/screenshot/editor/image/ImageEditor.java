...
public class ImageEditor implements UIComponentWrapper2 {
  ...
  private final List<AnAction> myLeadingActions = Collections15.arrayList();
  ...
  public ImageEditor(Configuration config, BufferedImage image) {
    assert image != null;
  ...
  public UIComponentWrapper2 createToolPanel(ImageDesk desk, Configuration config, List<ImageTool> tools) {
    return new EditorToolbar(desk, config, tools);
  }
  ...
  public JComponent getComponent() {
    if (myWholePanel == null) {
      ...
      myToolPanel = createToolPanel(myImageDesk, myConfig, myTools);
      setupWholePanel();
  ...
  private class EditorToolbar implements UIComponentWrapper2 {
    ...
    public EditorToolbar(ImageDesk imageDesk, Configuration config, List<ImageTool> tools) {
      myZoomPanel = new ZoomPanel(myImageDesk.getWorkingImage(), imageDesk);
      setupSidebar(config, tools);
    ...
    private void setupSidebar(Configuration config, List<ImageTool> tools) {
