...
public class DatabaseWrapperTests extends BaseTestCase {
  public static final DBNamespace NS = Engine.NS.subNs("tests");
  ...
  protected void setUp() throws Exception {
    ...
    wdb = new DatabaseWrapper(new TestWorkArea());
  }

  @Override
  protected void tearDown() throws Exception {
    ((DatabaseWrapper)wdb).stop();
    super.tearDown();
  }
  ...
  private static String readItems(LongList items, DBReader reader) {
    ...
    for (LongListIterator i = items.iterator(); i.hasNext();) {
      s.append(reader.getValue(i.next(), ATTR));
    }
    return s.toString();
  }
