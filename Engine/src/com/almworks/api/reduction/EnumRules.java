...
public class EnumRules {
  ...
  public static Rule[] collect(final Collection<DBAttribute> attributes) {
    Collect2 collect = new Collect2() {
      protected boolean shouldCollect(DBAttribute attribute) {
        return attributes.contains(attribute);
      }
    };
    return new Rule[]{new ItemHypercubeUtils.ReplaceIntersectionWithEqualsRule(attributes), collect};
  }

  public static abstract class Collect2 extends BaseCollectRule<MultiMap<Pair<Boolean, DBAttribute>, Long>> {
    public Collect2() {
      super(null);
    }

    protected ConstraintTreeElement applyResult(
      ConstraintTreeNode group, MultiMap<Pair<Boolean, DBAttribute>, Long> context)
    {
    ...
    protected abstract boolean shouldCollect(DBAttribute attribute);
  }
