...
public class DBStatusKey extends SystemKey<ItemWrapper.DBStatus> implements AutoAddedModelKey<ItemWrapper.DBStatus>{
  ...
  private static BoolExpr<DP> mod(Connection connection, DBReader reader) {
    return connection != null ? connection.getProvider().getPrimaryStructure().getLocallyChangedFilter() : ItemStorageAdaptor.modified(BadUtil.getMasterAttributes(reader));
  }

  private static BoolExpr<DP> conf(Connection connection, DBReader reader) {
    return connection != null ? connection.getProvider().getPrimaryStructure().getConflictingItemsFilter() : ItemStorageAdaptor.inConflict(BadUtil.getMasterAttributes(reader));
  }
