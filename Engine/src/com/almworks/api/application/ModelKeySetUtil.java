...
public class ModelKeySetUtil {
  ...
  public static Set<ModelKey<?>> collectSet(BitSet2 keys, Condition<ModelKey<?>> condition) {
    HashSet<ModelKey<?>> result = Collections15.hashSet();
    for (int i = keys.nextSetBit(0); i >= 0; i = keys.nextSetBit(i + 1)) {
      ModelKey<?> key = ModelKeySetUtil.getKey(i);
      if (key != null) {
        if (condition.isAccepted(key)) {
          result.add(key);
        }
      }
    }
