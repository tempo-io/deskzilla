...
public class ItemKeyCache {
  ...
  public ResolvedItem findExisting(long item) {
    synchronized (myLock) {
      try {
        return priFindExisting(item, null);
...
// create the key - not under a lock!
      badMessage = null;
      try {
        key = factory.createResolvedItem(item, reader);
        if (key == null) {
  ...
  public static ItemKeyCache getFromContainer(ComponentContainer container) {
    NameResolver resolver = container.getActor(NameResolver.ROLE);
    assert resolver != null : container;
    return resolver.getCache();
  }
}
