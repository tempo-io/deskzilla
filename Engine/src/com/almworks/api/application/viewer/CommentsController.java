...
public class CommentsController<T extends Comment> implements UIController<JPanel> {
  ...
  private Comparator<T> myOriginalComparator;
    ...
    JPanel panel)
  {
    ...
    myMenu.addAction(StdItemActions.ADD_COMMENT);
    myMenu.addAction(StdItemActions.REPLY_TO_COMMENT);
    myMenu.addAction(StdItemActions.EDIT_COMMENT);
    myMenu.addAction(StdItemActions.REMOVE_COMMENT);
  ...
  private <K, N extends TreeModelBridge<T>> void connectTree(final Lifespan lifespan, CommentsTree<T, K, N> structure, final ATree<N> tree)
  {
    ...
    myTable.getSelectionAccessor().addAWTChangeListener(lifespan, new ChangeListener() {
      public void onChange() {
        if (!lifespan.isEnded() && !changing[0] && tree.isDisplayable() && myTable.isDisplayable()) {
          CommentState<T> state = myTable.getSelectionAccessor().getSelection();
          if (state != null) {
            changing[0] = true;
            try {
  ...
  public CommentsController<T> setComparator(Comparator<T> comparator) {
  ...
  public CommentsController<T> addMenuAction(AnAction action, boolean afterSeparator) {
    if (afterSeparator)
      myMenu.addSeparator();
