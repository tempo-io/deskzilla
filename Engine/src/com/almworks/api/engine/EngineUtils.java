...
public class EngineUtils {
  ...
  public static String suggestConnectionNameByUrl(String url) {
    ...
    if (url.length() == 0)
      return null;
    if ("https://".indexOf(url) >= 0 || "http://".indexOf(url) >= 0) {
      // too short url
      return null;
    }
    // remove protocol
  ...
  public static Configuration getKludgeConfiguration(ActionContext context, String kludgeName)
    throws CantPerformException
  {
    // kludge!
    ComponentContainer container = context.getSourceObject(ComponentContainer.ROLE);
    ComponentContainer subcontainer = container.createSubcontainer(kludgeName);
    Kludge kludge = subcontainer.instantiate(Kludge.class);
    assert kludge.kludge != null;
    return kludge.kludge;
  }
  ...
  public static long getConnectionUser(Connection connection, DBReader reader) {
    if (connection == null || connection.getState().getValue() != ConnectionState.READY) return 0L;
    return getConnectionUser(connection.getConnectionRef(), reader);
  }

  /**
   * Call only when connection is READY.
   */
  public static long getConnectionUser(DBIdentifiedObject connectionItem, DBReader reader) {
    return Util.NN(reader.getValue(reader.assertMaterialized(connectionItem), Connection.USER), 0L);
  }
