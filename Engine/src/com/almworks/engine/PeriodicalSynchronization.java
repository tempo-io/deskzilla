...
class PeriodicalSynchronization implements Startable {
  ...
  private SyncParameters getConnectionSyncParameters(Connection connection, final long currentTime) {
    ...
    if (quick) {
      ...
      trackSyncProgressAndSetNextFullSyncTime(currentTime, syncStateModel, connectionID, thisVersion);
    }
  ...
  private void trackSyncProgressAndSetNextFullSyncTime(long currentTime, ScalarModel<SyncTask.State> syncStateModel,
    final String connectionID, final String thisVersion)
  {
    final long successNextTime = currentTime + getSuccessPause();
      ...
      public void onScalarChanged(ScalarModelEvent<SyncTask.State> event) {
        ...
        if (progress[0] == 0 && state == SyncTask.State.WORKING) {
          ...
          if (state == SyncTask.State.DONE) {
            progress[0] = 2;
            detach.detach();
            myConfiguration.setSetting(connectionID, successNextTime);
            myConfiguration.setSetting(VERSION_PREFIX + connectionID, thisVersion);
