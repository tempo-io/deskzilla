...
class ConnectionManagerImpl implements ConnectionManager {
  ...
  public Connection findByItem(long connectionItem) {
    try {
      return findByItem(connectionItem, false);
    } catch (InterruptedException e) {
      throw new CantGetHereException();
    }
  }

  public Connection findByItem(long connectionItem, boolean wait) throws InterruptedException {
    if (connectionItem == 0)
      return null;
    synchronized (myConnectionByItemCache) {
      ...
      if (result == null) {
        result = findConnection(connectionItem, wait);
        if (result != null) {
  ...
  private Connection findConnection(long connectionItem, boolean wait) throws InterruptedException {
//    if (wait) {
...
//    }
    ...
    if (descriptorId.length() != 0) {
      synchronized (this) {
        ...
        if (!descriptor.isEnabled()) {
          Log.warn("Provider " + descriptorId + " is disabled");
          return null;
        }
      }
    } else {
    ...
    throws ConfigurationException, ProviderDisabledException
  {
    ...
    if (!connection.isAllowedByLicense()) {
