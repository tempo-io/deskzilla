...
import com.almworks.api.application.field.RightCustomFields;
...
public abstract class CommonIssueViewer implements UIComponentWrapper2 {
  ...
  public CommonIssueViewer(Configuration config, MetaInfo metaInfo) {
    myConfig = config;
    myMetaInfo = metaInfo;
  }
  ...
  private void addTags(final LeftFieldsBuilder fields) {
    fields.addSeparator();
    fields.addLine(TagsModelKey.INSTANCE.getDisplayableName() + ':',
      new AbstractListCell(fields.getPresentation(), "tags") {
        protected List<TableRendererCell> createCells(RendererContext context) {
          List<ResolvedTag> tagsData = LeftFieldsBuilder.getModelValueFromContext(context, TagsModelKey.INSTANCE);
          List<TableRendererCell> result = Collections15.arrayList();
          if (tagsData != null) {
            List<ResolvedTag> tags = tagsData;
            for (ResolvedTag tag : tags) {
              result.add(new TextWithIconCell(FontStyle.BOLD, tag.getDisplayName(), tag.getIcon()));
            }
          }
          return result;
        }
      ...
      public Boolean invoke(RendererContext context) {
        List<ResolvedTag> tagsData = LeftFieldsBuilder.getModelValueFromContext(context, TagsModelKey.INSTANCE);
        return tagsData != null && !tagsData.isEmpty();
      }
    });
  }
  ...
  protected WidthDrivenComponent createRightCustomFieldFormlets(ModelKey<List<ModelKey<?>>> key,
    Configuration settings)
  {
    final WidthDrivenComponent widthDrivenComponent =
      RightCustomFields.createComponent(key, settings.getOrCreateSubset("rightCustomFields"),
        getDocumentFormAugmentor(), new Procedure<Highlightable>() {
