...
public class TitledFormlet extends WidthDrivenStackedCouple {
  ...
  private final FormletTitle myTitle;
  private final ChangeListener myListener = new MyListener();

  public TitledFormlet(Formlet formlet, String title, int index) {
    this(formlet, new FormletTitle(formlet, title, index));
  }

  private TitledFormlet(Formlet formlet, FormletTitle title) {
    super(new WidthDrivenComponentAdapter(title), formlet.getContent());
    assert title.getFormlet() == formlet;
    myFormlet = formlet;
    myTitle = title;
  }
