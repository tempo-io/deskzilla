...
public class AttachmentsTable<T extends Attachment> implements TableTooltipProvider {
  ...
  public AttachmentsTable(AttachmentDownloadStatus<T> status) {
    this(status, new ASortedTable<T>());
  }

  public AttachmentsTable(AttachmentDownloadStatus<T> status, ASortedTable<T> table) {
    myStatus = status;
    myTable = table;
    myTable.setDataRoles(AttachmentsEnv.ATTACHMENT);
  ...
  public void initialize(List<AttachmentProperty<? super T, ?>> properties, Configuration config, @Nullable AnAction deleteAction) {
    initialize(properties, config, AttachmentsPanel.createPopupMenu(deleteAction));
  }
  ...
  public void setTooltipProvider(AttachmentTooltipProvider<T> tooltipProvider) {
