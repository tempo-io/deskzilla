...
public class AttachmentContent implements UIComponentWrapper {
  ...
  private static final long MAXIMUM_FILE_SIZE = 25 * 1024 * 1024;
  ...
  public static boolean isFileShowable(File file, String mimeType) {
    return createFactory(file, mimeType) != null;
  }
  ...
  private static String guessMimeType(File file) {
    //todo duplicating AttachmentUtils
    if (file == null)
      return null;
  ...
  private void loadFileAndShowComponent() {
    ...
    try {
      if (!FileUtil.isEmpty(myFile)) {
        if (myFile.length() > MAXIMUM_FILE_SIZE)
          throw new IOException(myFile + " is too large");
