...
class ItemTreeBuilder implements IntIntFunction {
   ...
   */
  private ListLongMap myItemParents;
  ...
  private void removeInvalid() {
    for (ListLongMap.Iterator ii = myItemParents.iterator(); ii.hasNext(); ) {
      ii.next();
      // todo collect ranges of invalid items and remove with one operation
      if (ii.value() == INVALID_ITEM) {
        ii.remove();
      }
    }
  }
    ...
    LongList updatedSorted)
  {
    ...
    long nextsgs = sgii.hasNext() ? sgii.next() : -1;
    long nextsgt = sgii.hasNext() ? sgii.next() : -1;
    assert (nextsgs < 0) == (nextsgt < 0) : nextsgs + " " + nextsgt + " " + graph + " " + sgii;

    LongListIterator ugii = graphUpdate.iterator();
    long nextugs = ugii.hasNext() ? ugii.next() : -1;
    long nextugt = ugii.hasNext() ? ugii.next() : -1;
    ...
    while (nextsgs >= 0 || nextugs >= 0) {
      while (nextsgs >= 0 && (nextsgs <= nextugs || nextugs < 0)) {
        ...
        while (nextsgs == current) {
          ...
          nextsgs = sgii.hasNext() ? sgii.next() : -1;
          nextsgt = sgii.hasNext() ? sgii.next() : -1;
          assert (nextsgs < 0) == (nextsgt < 0) : nextsgs + " " + nextsgt + " " + graph + " " + sgii;
        }
      }
      while (nextugs >= 0 && (nextugs < nextsgs || nextsgs < 0)) {
        result.add(nextugs);
        result.add(nextugt);
        nextugs = ugii.hasNext() ? ugii.next() : -1;
        nextugt = ugii.hasNext() ? ugii.next() : -1;
        assert (nextugs < 0) == (nextugt < 0) : nextugs + " " + nextugt + " " + graphUpdate + " " + ugii;
      }
    }
    return result;
