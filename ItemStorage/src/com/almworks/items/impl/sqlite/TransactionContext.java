...
public class TransactionContext implements TableResolver {
  ...
  private Map<? extends TypedKey, ?> myTransactionCache = Collections15.hashMap();
  ...
  public void flushChangedItemsICN() throws SQLiteException {
    ...
    while (true) {
      ...
      if (changed.isEmpty())
        break;
      LongList flushed = totalCollector.toTemporaryReadOnlySortedCollection();
      workingSet.addAll(new SortedLongListMinusIterator(changed.iterator(), flushed.iterator()));
      if (workingSet.isEmpty())
        break;
      totalCollector.mergeFromSortedCollection(workingSet);
      propagateChanges(propagating, workingSet);
    }
    updateItemCN(totalCollector.toSortedCollection());
  }
