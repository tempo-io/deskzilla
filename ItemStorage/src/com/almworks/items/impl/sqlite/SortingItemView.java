...
public class SortingItemView implements ItemView {
  ...
  protected void reload(LongList items, @Nullable LongList itemGraph, @Nullable AbstractGroupsDist groups) {
    IntList depths = null;
    DiffIndexedLongListDecorator itemTree = null;
    if (itemGraph != null) {
      if (itemGraph.isEmpty()) {
        SameValuesIntList zeroes = new SameValuesIntList();
        zeroes.insertMultiple(0, 0, items.size());
        itemTree = new DiffIndexedLongListDecorator(items, zeroes);
        depths = zeroes;
      } else {
    ...
    if (removedCount + updatedCount > 0) {
      ...
      for (LongListIterator ii = current.iterator(); ii.hasNext();) {
        ...
        if (removedCount > 0 && removed.binarySearch(item) >= 0) {
          indexesRU.add(ii.lastIndex());
        } else if (updatedCount > 0 && updated.binarySearch(item) >= 0) {
          indexesRU.add(ii.lastIndex());
        }
      }
      ReadonlyIntListRemovingDecorator.prepareSortedIndices(indexesRU);
      removeDecorator = ReadonlyLongListRemovingDecorator.createFromPrepared(current, indexesRU);
  ...
  private abstract static class ViewChange implements Event {
    ...
    @Nullable
    public abstract DiffIndexedLongListDecorator getItemTree();

    @Nullable
    public abstract AbstractGroupsDist getGroups();

    public LongList getItems() {
      DiffIndexedLongListDecorator tree = getItemTree();
  ...
  protected static class ItemViewFrame {
    ...
    public synchronized void flatten() {
      ...
      DiffIndexedLongListDecorator tree = myEvent.getItemTree();
      // todo effective flattening in-place
      LongList items = myEvent.getItemList();
      LongList newItems;
      if (items instanceof SegmentedIntArray) {
        newItems = items;
      } else {
...
//        IntCollectionFactory.dispose(items);
        if (tree != null) {
          tree = new DiffIndexedLongListDecorator(newItems, tree.getIndexes());
        }
      }
  ...
  static class ReplaceEvent extends ViewChange {
    ...
    public void getRange(int firstIndex, ItemAcceptor acceptor) {
      ...
      while (it.hasNext()) {
        if (!acceptor.accept(firstIndex, it.next(), ItemState.INSERTED))
          return;
        firstIndex++;
      }
    }
    ...
    @Nullable
    public DiffIndexedLongListDecorator getItemTree() {
  ...
  static class UpdateEvent extends ViewChange {
    ...
    @Nullable
    private final DiffIndexedLongListDecorator myItemTree;
    @Nullable
    ...
    UpdateEvent(LongList prevItemList, LongList prevItems, LongList removedItems,
      ...
      @Nullable LongListInsertingDecorator insertDecorator, DiffIndexedLongListDecorator itemTree, WritableIntList depths,
    ...
    public int getNewIndex(int oldIndex) {
      if (myItemTree == null) {
        ...
        backward.move(ss);
        while (forward.hasNext() || backward.lastIndex() > 0) {
          if (forward.hasNext() && forward.next() == item)
            return forward.lastIndex();
          if (backward.lastIndex() > 0) {
            backward.move(-1);
            if (backward.get(0) == item)
              return backward.lastIndex();
          }
        }
        return -1;
      }
  ...
  private static class TreeInfo {
    ...
    private DiffIndexedLongListDecorator myItemTree;
    ...
    public TreeInfo build() {
      ...
      myItemTree = new DiffIndexedLongListDecorator(myItems, treeIndexes);
