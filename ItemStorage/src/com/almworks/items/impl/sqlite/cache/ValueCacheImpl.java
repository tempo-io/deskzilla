...
import com.almworks.util.collections.arrays.*;
...
class ValueCacheImpl implements ValueCache {
  ...
  private final ParallelLongList myItemsAndReindex = new ParallelLongList(new LongArray(), 2);
  ...
  private boolean checkItems() {
    assert Thread.holdsLock(myManager.getLock());
    ParallelLongList.Iterator it = myItemsAndReindex.iterator(0);
  ...
  public void addAttributes(List<? extends Attribute> attributes) {
    synchronized (myManager.getLock()) {
      ...
      for (ParallelLongList.Iterator it = myItemsAndReindex.iterator(0); it.hasNext();) {
        it.next(itemReindex);
