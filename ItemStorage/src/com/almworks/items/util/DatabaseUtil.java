...
public class DatabaseUtil {
  ...
  public static void query(final DBFilter view, ThreadGate gate, Procedure<LongList> proc) {
    Database.require().read(DBPriority.BACKGROUND, new ReadTransaction<LongList>() {
  ...
  public static List<AttributeMap> queryMap(final BoolExpr<DP> query) {
    return Database.require().readForeground(new ReadTransaction<List<AttributeMap>>() {
      @Override
      public List<AttributeMap> transaction(DBReader reader) throws DBOperationCancelledException {
        ...
        List<AttributeMap> result = Collections15.arrayList();
        for (int i = 0; i < items.size(); i++) result.add(reader.getAttributeMap(items.get(i)));
        return result;
      }
    }).waitForCompletion();
  }
  ...
  private static List<AttributeMap> map(final long... artifacts) {
    return Database.require().read(DBPriority.BACKGROUND, new ReadTransaction<List<AttributeMap>>() {
      @Override
  ...
  public static AttributeMap loadMap(final long artifact) {
    return Database.require().read(DBPriority.BACKGROUND, new ReadTransaction<AttributeMap>() {
