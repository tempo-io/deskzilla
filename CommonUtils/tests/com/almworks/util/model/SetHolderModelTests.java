...
public class SetHolderModelTests extends BaseTestCase {
  ...
  public static void main(String[] args) throws InterruptedException {
    final SetHolderModel<Integer> model = new SetHolderModel<Integer>();
    Updater[] updaters = new Updater[20];
    Image[] images = new Image[10];
    try {
      for (int i = 0; i < updaters.length; i++) {
        updaters[i] = new Updater(model);
        updaters[i].start();
        Thread.yield();
      }
      for (int i = 0; i < images.length; i++) {
        synchronized (images) {
          images[i] = new Image(images);
        }
        model.addInitListener(Lifespan.FOREVER, ThreadGate.STRAIGHT, images[i]);
        Thread.yield();
      }
      while (true) {
        Thread.sleep(1000);
        for (Updater updater : updaters) updater.pauseUpdate();
        for (Updater updater : updaters) updater.waitPaused();
        List<Integer> list = model.copyCurrent();
        Collections.sort(list);
        System.out.println(TextUtil.separateToString(list, ","));
        for (Image image : images) image.checkImage(list);
        for (Updater updater : updaters) updater.resumeUpdate();
