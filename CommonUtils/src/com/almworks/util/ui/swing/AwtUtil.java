...
public class AwtUtil {
  ...
  private static Insets priGetTempInsets(JComponent component) {
    Insets insets;
    if (EventQueue.isDispatchThread()) {
      component.getInsets(TEMP_INSETS);
      insets = TEMP_INSETS;
    } else
      insets = component.getInsets(new Insets(0, 0, 0, 0));
    return insets;
  }
  ...
  public static int getInsetWidth(JComponent component) {
    if (component == null)
      return 0;
    return getInsetWidth(priGetTempInsets(component));
  }
  ...
  public static int getInsetHeight(JComponent component) {
    if (component == null)
      return 0;
    return getInsetHeight(priGetTempInsets(component));
  }
  ...
  public static Insets uniteInsetsFromTo(JComponent decendant, Container ancestor) {
    Insets result = decendant.getInsets(new Insets(0, 0, 0, 0));
    Container c = decendant;
    while (true) {
      Container parent = c.getParent();
      if (parent == null) {
        assert false : "Decendant: " + decendant + " acestor: " + ancestor;
        break;
      }
      if (parent instanceof JComponent)
        priUniteInsets(result, priGetTempInsets((JComponent) parent));
      if (parent == ancestor)
        break;
      c = parent;
    }
  ...
  public static Shape clipClientArea(Graphics g, JComponent component) {
    ...
    Insets insets = priGetTempInsets(component);
