...
public class Functional {
  ...
  public static <T> Iterable<T> concat(@Nullable Iterable<? extends T>... iterables) {
    return selectMany((List) Arrays.asList(iterables), Convertor.<Iterable<T>>identity());
  }

  public static <T0, T1, T2> Function<T0, Function<T1, T2>> curry(final Function2<T0, T1, T2> f) {
    return new Function<T0, Function<T1, T2>>() {
      @Override
      public Function<T1, T2> invoke(final T0 a0) {
        return new Function<T1, T2>() {
          @Override
          public T2 invoke(T1 a1) {
