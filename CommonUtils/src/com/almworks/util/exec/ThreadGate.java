...
public abstract class ThreadGate {
  ...
  public static final ImmediateThreadGate FX_IMMEDIATE = new FXImmediateThreadGate();
  ...
  public static void executeLong(Object key, final InterruptableRunnable runnable) {
    LONG(key).execute(new InterruptableWrapper(runnable));
  }

  public static void executeLong(InterruptableRunnable runnable) {
    LONG.execute(new InterruptableWrapper(runnable));
  }
  ...
  private static class InterruptableWrapper implements Runnable {
    private final InterruptableRunnable myRunnable;

    public InterruptableWrapper(InterruptableRunnable runnable) {
      myRunnable = runnable;
    }
