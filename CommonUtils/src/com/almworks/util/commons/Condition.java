...
public abstract class Condition <T> {
  ...
  public boolean hasAny(Collection<? extends T> collection) {
  ...
  public Pair<List<T>, List<T>> split(@Nullable Collection<? extends T> src) {
    List<T> sat = arrayList();
    List<T> not = arrayList();
    if (src != null) {
      for (T t : src) {
        if (isAccepted(t)) sat.add(t);
        else not.add(t);
      }
    }
