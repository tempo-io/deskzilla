...
public class StringUtil {
  ...
  public static String normalizeWhitespace(String message) {
    ...
    return StringUtil.implode(result, StringUtil.LINE_SEPARATOR).trim();
  }
  ...
  public static List<AlphaNumeric> alphaNumericSplit(String str) {
    if(str == null || str.isEmpty()) {
      return Collections15.emptyList();
    }

    final List<AlphaNumeric> segments = Collections15.arrayList();
    final StringBuilder segment = new StringBuilder();
    Boolean segmentIsNumeric = null;

    for(int i = 0; i < str.length(); i++) {
      final char c = str.charAt(i);
      final Boolean numeric = (c >= '0' && c <= '9') ? Boolean.TRUE : Boolean.FALSE;
      if(!numeric.equals(segmentIsNumeric) && segmentIsNumeric != null) {
        segments.add(segmentIsNumeric ? AlphaNumeric.number(segment) : AlphaNumeric.alpha(segment));
        segment.setLength(0);
      }
      segment.append(c);
      segmentIsNumeric = numeric;
    }

    if(segmentIsNumeric != null) {
      segments.add(segmentIsNumeric ? AlphaNumeric.number(segment) : AlphaNumeric.alpha(segment));
    }
  ...
  public static class AlphaNumeric {
    public static AlphaNumeric alpha(CharSequence s) {
      return new AlphaNumeric(s.toString(), null);
    }

    public static AlphaNumeric number(CharSequence s) {
      return new AlphaNumeric(null, s.toString());
    }

    private final String myAlpha;
    private final BigInteger myNumber;

    public AlphaNumeric(String alpha, String number) {
      assert isGood(alpha) != isGood(number) : "" + alpha + " " + number;
      myAlpha = isGood(alpha) ? alpha : null;
      myNumber = isGood(number) ? new BigInteger(number, 10) : null;
    }
    ...
    public boolean equals(Object o) {
      ...
      if(o instanceof AlphaNumeric) {
        final AlphaNumeric that = (AlphaNumeric) o;
        return Util.equals(myAlpha, that.myAlpha)
          && Util.equals(myNumber, that.myNumber);
      }
      return false;
    }
    ...
    @Override
    public int hashCode() {
      int result = myAlpha != null ? myAlpha.hashCode() : -37;
      result = 31 * result + (myNumber != null ? myNumber.hashCode() : -41);
      return result;
    }
  }
