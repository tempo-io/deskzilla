...
//    myOwner = owner;
  ...
  protected MyDialog createWindow(Detach disposeNotification) {
    Window owner = null;
    if (!myNullOwner) {
  ...
  public void insertContent(MyDialog dialog, UIComponentWrapper content) {
    ...
    if(!Env.isMac()) {
      myOkButton = dialog.addAction(myOkAction, true);
//      if (myOkButton != null)
//        UIUtil.adjustFont(myOkButton, -1, Font.BOLD, false);
      dialog.setCancelAction(myCancelAction);
...
*/
    ...
    public void setCancelAction(AnAction action) {
      ...
      myCancelButton = addAction(action, false);
    }

    @Nullable
    public AActionButton addAction(AnAction action, boolean isDefault) {
      if (action == null)
        return null;
