...
public class Wizard {
  /** "Next" button text. */
  public static final NameMnemonic NEXT = NameMnemonic.parseString(Env.isMac() ? "Continue" : "&Next >");
  ...
  private void closeWindow(String pageID) {
    ...
    final WindowController ec = myBuilder.getWindowContainer().getActor(WindowController.ROLE);
    if(ec != null) {
      ec.close();
    }
  }
  ...
  private static abstract class WizardAction implements AnAction {
    ...
    public void update(UpdateContext context) throws CantPerformException {
      ...
      adjustButtonSize(comp);
      focusButtonIfNeeded(comp);
    ...
    private void adjustButtonSize(Component comp) {
      if(comp instanceof JButton) {
        comp.setMinimumSize(getButtonMinSize());
        comp.setPreferredSize(getButtonMinSize());
        comp.setMaximumSize(getButtonMinSize());
      }
    }

    private void focusButtonIfNeeded(Component comp) {
      if(comp instanceof JButton && myFocus) {
  ...
  private class NextAction extends WizardAction {
    ...
    public void perform(ActionContext context) throws CantPerformException {
      final WizardPage page = myPageMap.get(myPageID);
      page.nextInvoked();
